 ...
 * </p>
 * <ul>
 * <li>v1.2.0 (2015-12-19) - Se agrego soporte para la concurrencia y el metodo nextGaussian.
 * </li>
 * </ul>
 * <ul>
 * <li>v1.1.0 (2015-10-17) - Se agrego el metodo nextLong(long n).
 * </li>
 * </ul>
 * <ul>
 * <li>v1.0.0 (2015-07-31) - Version Inicial.</li>
 * </ul>
 */
public strictfp class KAOSrandPlus {

    // Lock para el manejo de concurrencia
    private ReentrantLock lock = new ReentrantLock();

    private static final int ATTRACTORSCLASS = 2;
    private static final int NUMATTRACTORS = 40;
    private static final int LIMIT = 25;

    private Atractor[] Atractores;
     ...
     */
    public KAOSrandPlus() {
        int i, AttSelect, AttType;

        // Desactiva los saltos de muestreo fijo de los atractores
        Jump = -1;
        Mask = 0x7FFFFFFFFFFFFFFFL;
        rnd = new MersenneTwisterPlus();
        Atractores = new Atractor[NUMATTRACTORS];
        for (i = 0; i < Atractores.length; i++) {
            AttSelect = i % ATTRACTORSCLASS;
            switch (AttSelect) {
                case 0:
                    Atractores[i] = new AtractorLorenz();
                    if (i < ((AtractorLorenz) Atractores[i]).getNumAtractorTypes()) {
                        AttType = i % ((AtractorLorenz) Atractores[i]).getNumAtractorTypes();
                    } else {
                        AttType = (i + 1) % ((AtractorLorenz) Atractores[i]).getNumAtractorTypes();
                    }
                    ((AtractorLorenz) Atractores[i]).SetAtractorType(AttType);
                    break;
                case 1:
                    Atractores[i] = new AtractorPickover();
                    if (i < ((AtractorPickover) Atractores[i]).getNumAtractorTypes()) {
                        AttType = i % ((AtractorPickover) Atractores[i]).getNumAtractorTypes();
                    } else {
                        AttType = (i + 1) % ((AtractorPickover) Atractores[i]).getNumAtractorTypes();
                    }
                    ((AtractorPickover) Atractores[i]).SetAtractorType(AttType);
                    break;
                default:
                    Atractores[i] = new AtractorLorenz();
                    if (i < ((AtractorLorenz) Atractores[i]).getNumAtractorTypes()) {
     ...
     * @param Sprk
     */
    public KAOSrandPlus(Sparker Sprk) {
        int i, AttSelect, AttType;
